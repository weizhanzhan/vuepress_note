---
home: true
heroImage: /a.jpg
actionText: follow me 🏄
heroText: manda
tagline: Love technology, enjoy life
actionLink: /note/
features:
- title: 📑笔记记录
  details: JavaScript,Vue,React,Css等系列内容的开发内容记录
- title: 🚀实验室
  details: 自己的项目集和好玩的一些东西
- title: 📚工作总结
  details: 工作中的问题总结及知识点归纳

footerColumn: 4
footerWrap: 
- headline: 🌿UI库
  items:
  - title: Element
    link: https://element.eleme.cn/#/zh-CN
    details: Vue
  - title: Ant Design
    link: https://ant.design/index-cn
    details: React
  - title: Ant Design Vue
    link: https://www.antdv.com/docs/vue/introduce-cn/
    details: Vue
- headline: ⚓资源链接
  items:
  - title: 掘金
    link: https://juejin.im/timeline
  - title: Ant Design Vue
    link: https://antdv.com/
- headline: 💬社区交流
  items:
  - title: VuePress 社区
    link: https://zhuanlan.zhihu.com/press-of-vue/
    details: 知乎专栏
  - title: VuePress 社区
    link: https://shang.qq.com/wpa/qunwpa?idkey=fce20f9d7a5620dbb261b31b6bd01f726c9e24e7697fcba4ea7927d5dc971ac5
    details: QQ交流群
footer: MIT Licensed | Copyright © 2018-present zhanwei | 苏ICP备19015245号-2
---